...
import {
  ...
  AlertTriangle, Clock, Receipt, BarChart3
} from "lucide-react";
...
async function StatsCards() {
  const [productCount, transactionCount, userCount, totalRevenue, categoryCount] = await Promise.all([
  ...
  const todayCount = todayRevenue._count || 0;
  const todaySales = todayRevenue._sum.total || 0;

  const stats = [
    {
      label: "Total Produk", value: productCount.toString(),
      sub: `${categoryCount} kategori`, icon: Package,
      color: "bg-blue-500/10 text-blue-400", href: "/admin/produk",
    },
    {
      label: "Total Transaksi", value: transactionCount.toString(),
      sub: `${todayCount} hari ini`, icon: ShoppingCart,
      color: "bg-emerald-500/10 text-emerald-400", href: "/admin/transaksi",
    },
    {
      label: "Total Pengguna", value: userCount.toString(),
      sub: "akun aktif", icon: Users,
      color: "bg-purple-500/10 text-purple-400", href: "/admin/pengguna",
    },
    {
      label: "Total Pendapatan", value: formatRupiah(totalRevenue._sum.total || 0),
      sub: `Hari ini: ${formatRupiah(todaySales)}`, icon: TrendingUp,
      color: "bg-amber-500/10 text-amber-400", href: "/admin/transaksi",
    },
  ];
    ...
    <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6" aria-label="Ringkasan statistik">
      {stats.map((stat) => (
        <Link key={stat.label} href={stat.href}
          className="card p-4 hover:bg-surface-container-high transition-colors group block">
...
async function TodayActivity() {
  ...
  const [todayTransactions, todayRevenue] = await Promise.all([
...
async function TopProducts() {
  const topProductIds = await prisma.transactionItem.groupBy({
    by: ["productId"],
  ...
  const ids = topProductIds.map(tp => tp.productId).filter(Boolean) as number[];
        ...
        {topProductIds.map((tp, i) => (
          <div key={tp.productId ?? i} className="flex items-center justify-between">
            <div className="min-w-0 flex items-center gap-2">
              ...
              <p className="text-xs text-zinc-300 truncate">{productMap.get(tp.productId ?? 0) ?? `#${tp.productId}`}</p>
            </div>
  ...
  if (recent.length === 0) {
    return (
      <section className="card" aria-label="Transaksi terbaru">
        <div className="p-4 text-center text-zinc-500 text-sm">Belum ada transaksi.</div>
      </section>
    );
  }
