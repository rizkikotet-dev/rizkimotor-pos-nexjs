    ...
    <div className="h-screen flex flex-col">
      ...
      <header className="bg-white/90 backdrop-blur-lg border-b border-surface-200/80 px-3 sm:px-4 py-2.5 flex items-center justify-between flex-shrink-0 z-20">
        ...
        <div className="flex items-center gap-1">
          <a href="/pos/riwayat" className="btn-ghost text-xs px-2 py-1.5 rounded-lg">
            <History className="h-4 w-4" /><span className="hidden sm:inline">Riwayat</span>
          </a>
          {user.role === "ADMIN" && (
            <a href="/admin" className="btn-ghost text-xs px-2 py-1.5 rounded-lg">
              <Package className="h-4 w-4" /><span className="hidden sm:inline">Admin</span>
            </a>
          )}
          <div className="border-l border-surface-200 pl-2 ml-1 flex items-center gap-2">
            ...
            <button onClick={() => signOut({ callbackUrl: "/login" })} className="text-red-500 hover:bg-red-50 p-1.5 rounded-lg transition-colors" title="Keluar">
                ...
                {filteredProducts.map((p) => {
                  ...
                  return (
                    <button key={p.id} onClick={() => handleProductClick(p)} disabled={out}
                      className="bg-white border border-surface-200 rounded-xl p-2 text-left hover:border-brand-400 hover:shadow-soft disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 group">
                      ...
                      <p className="text-xs font-semibold text-surface-900 line-clamp-2 leading-tight mb-0.5 group-hover:text-brand-600 transition-colors">{p.name}</p>
        ...
        <div className="w-full lg:w-96 bg-white border-t lg:border-t-0 lg:border-l border-surface-200/80 flex flex-col flex-shrink-0">
          ...
          <div className="flex-1 overflow-y-auto p-2 space-y-1 min-h-[80px]">
            {cart.length === 0 ? (
              <div className="text-center py-10 text-surface-400 text-sm">
                <ShoppingCart className="h-8 w-8 mx-auto mb-2 text-surface-300" />
                Keranjang kosong
              </div>
            ) : cart.map((item) => (
              <div key={item.id} className={`rounded-xl p-2.5 flex gap-2 ${item.isManual ? "bg-purple-50 border border-purple-200" : "bg-surface-50"}`}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-1">
                    ...
                    {item.isManual && <PenSquare className="h-3 w-3 text-purple-500 flex-shrink-0" />}
                  </div>
                ...
                <div className="flex flex-col items-end gap-1">
                  <button onClick={() => removeItem(item.id)} className="text-red-400 hover:text-red-600 p-0.5 transition-colors">
                    <X className="h-3 w-3" />
                  </button>
                  <div className="flex items-center gap-0.5 bg-white border border-surface-200 rounded-lg">
                    <button onClick={() => updateQty(item.id, -1)} className="px-1.5 py-0.5 hover:bg-surface-100 rounded-l-lg transition-colors">
                      <Minus className="h-3 w-3" />
                    </button>
                    <span className="text-xs font-semibold px-1.5 min-w-[24px] text-center">{item.quantity}</span>
                    <button onClick={() => updateQty(item.id, 1)} className="px-1.5 py-0.5 hover:bg-surface-100 rounded-r-lg transition-colors">
          ...
          <div className="px-3 pt-2 flex-shrink-0">
            <button onClick={() => setShowManualForm(!showManualForm)}
              className="w-full inline-flex items-center justify-center gap-1.5 text-xs font-semibold text-purple-700 bg-purple-50 hover:bg-purple-100 border border-purple-200 rounded-xl py-2 transition-all">
          ...
          {/* Manual item form */}
          {showManualForm && (
            <div className="px-3 py-2.5 bg-purple-50 border-t border-purple-200 space-y-2 flex-shrink-0">
              <input type="text" value={manualName} onChange={(e) => setManualName(e.target.value)}
                placeholder="Nama barang (mis: Jasa Service)"
                className="w-full px-3 py-2 border border-purple-300 rounded-xl text-xs focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500" />
              <div className="flex gap-2">
                <input type="text" inputMode="numeric" value={manualPrice} onChange={(e) => setManualPrice(e.target.value)}
                  placeholder="Harga"
                  className="flex-1 px-3 py-2 border border-purple-300 rounded-xl text-xs font-mono focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500" />
                <input type="number" min="1" value={manualQty} onChange={(e) => setManualQty(parseInt(e.target.value) || 1)}
                  className="w-16 px-2 py-2 border border-purple-300 rounded-xl text-xs text-center focus:outline-none focus:ring-2 focus:ring-purple-500/20 focus:border-purple-500" />
                <button onClick={addManualItem}
                  className="bg-purple-600 text-white px-3 py-2 rounded-xl text-xs font-semibold hover:bg-purple-700 transition-colors">
                  Tambah
                </button>
              </div>
            </div>
          ...
          <div className="border-t border-surface-200 p-3 space-y-2.5 bg-surface-50 flex-shrink-0">
            ...
            <div className="flex justify-between items-baseline">
              ...
              <span className="text-xl font-bold text-brand-600">{formatRupiah(total)}</span>
            </div>
            <div>
              <label className="block text-xs font-semibold text-surface-600 mb-1.5">Bayar (Rp)</label>
              <input type="text" inputMode="numeric" value={payment} onChange={(e) => setPayment(e.target.value.replace(/\D/g, ""))}
                placeholder="0" className="input font-mono text-base" />
            </div>
            <div className="grid grid-cols-3 gap-1.5">
              <button onClick={setExactAmount} className="text-xs font-semibold bg-white border border-surface-200 hover:bg-surface-50 px-2 py-2 rounded-lg transition-colors">
                Pas
              </button>
              {QUICK_AMOUNTS.map((amt) => (
                <button key={amt} onClick={() => setQuickAmount(amt)}
                  className="text-xs font-semibold bg-white border border-surface-200 hover:bg-surface-50 px-2 py-2 rounded-lg transition-colors">
            ...
            {payment && parseInt(payment) > 0 && (
              <div className="flex justify-between text-sm pt-2 border-t border-surface-200">
                ...
                <span className={`font-bold ${change < 0 ? "text-red-600" : "text-emerald-600"}`}>
                  {formatRupiah(change)}
                </span>
              </div>
            )}
            <button onClick={checkout} disabled={!canCheckout}
              ...
              {checkoutLoading ? (
                ...
                <><Receipt className="h-4 w-4" />Bayar & Cetak Struk</>
              )}
            </button>
          </div>
        </div>
      ...
      {priceModal && (
        <div className="fixed inset-0 z-50 bg-surface-900/50 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in" onClick={() => setPriceModal(null)}>
          <div className="bg-white rounded-2xl p-6 w-full max-w-xs shadow-lifted animate-scale-in" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-2">
              <button onClick={() => { addToCart(priceModal, priceModal.price); setPriceModal(null); }}
                className="w-full text-left px-4 py-3.5 border border-surface-200 rounded-xl hover:bg-surface-50 transition-all flex items-center justify-between">
              ...
              <button onClick={() => { addToCart(priceModal, priceModal.priceReseller); setPriceModal(null); }}
                className="w-full text-left px-4 py-3.5 border-2 border-brand-500 bg-brand-50 rounded-xl hover:bg-brand-100 transition-all flex items-center justify-between">
            ...
            <button onClick={() => setPriceModal(null)}
              className="w-full text-center text-xs text-surface-500 hover:text-surface-700 mt-4 py-2 font-medium transition-colors">
              Batal
            </button>
          </div>
        </div>
